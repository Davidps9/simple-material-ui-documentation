...
Here is the type for the `checked` prop that can be used with the [Switch](/reference/switch/#checked) component:
```tsx
true | false;
```
...
Here is the type for the `disabled` prop that can be used with the [Switch](/reference/switch/#disabled) component:
```tsx
true | false;
```
